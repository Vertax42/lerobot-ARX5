...
Opening each camera's encoder — the PyAV container plus the codec context — is relatively expensive (tens of milliseconds). If this happened lazily on the episode's first frame, that frame would badly overrun the `fps` budget and show up as a startup spike in the recorded timestamps.

To avoid this, the record loop calls `dataset.prepare_episode_recording()` once before each episode. It starts the encoder threads and opens their codec contexts **up front**, using each video key's declared `(H, W, C)` shape, and blocks until every encoder reports ready. By the time the first frame is captured the encoders are already hot, so `add_frame()` no longer pays the initialization cost on frame 0. A lazy first-frame start remains as a defensive fallback for callers that don't pre-warm.
