...
def extract_flexiv_joint_features(obs):
    """提取关节位置、速度、力矩
    """
    joint_features = {}
    for key, value in obs.items():
        if key.endswith(".pos") or key.endswith(".vel") or key.endswith(".effort"):
            joint_features[key] = value
...
def flexiv_rizon4_record_loop(
    ...
    while timestamp < control_time_s:
        ...
        # Get teleop action (always runs to keep teleop state alive)
        ...
        if policy is not None and act_processed_policy is not None:
            ...
            robot_action_to_send = act_processed_policy
        else:
        ...
        if teleop.name == "pico4" and robot.name == "flexiv_rizon4":
            ...
            if reset_button and hasattr(robot, "reset_to_initial_position") and not resetting_this_frame:
                is_resetting = True
                resetting_this_frame = True
        ...
        # Skip only send_action during reset to avoid control mode conflict
        if not resetting_this_frame:
...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if teleop.name == "pico4" and robot.name == "flexiv_rizon4":
            reset_button = teleop.get_reset_button()
            if reset_button and hasattr(robot, "reset_to_initial_position"):
                is_resetting = True
        ...
        dataset_features = combine_feature_dicts(
            aggregate_pipeline_dataset_features(
                ...
                initial_features=create_initial_features(
                    action=robot._proprioception_ft
                ),
